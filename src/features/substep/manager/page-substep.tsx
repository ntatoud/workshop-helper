...
export function PageSubstep({
  ...
  return (
    <PageLayout>
      <PageLayoutTopbar
        startActions={
          <Button variant="ghost" title="Retour au step" size="icon">
            <Link
              to="/manager/workshops/$workshopId/steps/$stepId"
              params={{ workshopId, stepId }}
            >
        ...
        endActions={
          <Button variant="destructive" onClick={handleDeleteSubstep}>
            Delete Substep
          </Button>
        }
      >
        <PageLayoutTitle>Sous-étape : {substep.title}</PageLayoutTitle>
      </PageLayoutTopbar>
      <PageLayoutContent>
        {substep.description && (
          <p className="prose max-w-none p-2 pb-4">{substep.description}</p>
        )}
